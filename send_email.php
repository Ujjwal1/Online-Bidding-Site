...
if ($_POST['action'] == 'Send Message') {
	...
	if ($_POST['cannedMsg'] == 'WEEKLY_PICKS_REMINDER') {
		...
		$sql .= "(select count(p.pickID) from topherballpicks p inner join topherballschedule s on p.gameID = s.gameID where userID = u.userID and s.weekNum = " . $week . ") as userPicks ";
