...
if (!$isAdmin) {
	?>
		<a href="suicide_pick/home.php">SUICIDE PICK POOL</a>
	<?php
	...
	<?php
		...
		if($dbConnected)
			{	
				...
				$query_score= mysql_query("select points from topherballusers where userID = $userID");
				if($query_score)
					{
						if($result_score = mysql_fetch_array($query_score))
							{
								$points = $result_score['points'];
								if($points > 1)
								{
									echo "<strong>Score: ".$points." points</strong>";
								}
								else
								{
									echo "<strong>Score: ".$points." point</strong>";
								}
							}
					}
				else
...
?>
	<a href="suicide_pick/uploadResult.php">Upload Result </a> &nbsp; <a href="suicide_pick/defaultPick.php"> Set Default Pick</a> &nbsp;  <a href="updateSchedule.php"> Update Schedule </a> &nbsp; <a href="suicide_pick/lockMatch.php">Lock the Match</a> &nbsp; <a href="suicide_pick/reactivateUser.php">Reactivate User</a>
	<h1>Welcome, Admin!</h1>
