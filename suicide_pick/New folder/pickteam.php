...
{	
	if($dbConnected)
	{
		...
		$sql_activation = "select Activation from topherballusers where userID = $userID";
...
<?php		
		...
		{	$lock = explode("_",$_GET['lock']);
			$user= $lock[0];
			$game = $lock[1];
			$sql_lock = "update topherballpicks set `lock`=\"true\" where userID=\"$user\" and gameID=\"$game\"";
...
<?php		}
		...
		*/
		if(isset($_GET['delete']))			//True if user is asking to delete a temporarily pick.
		{
			...
			$sql_del = "delete from topherballpicks where userID= $userID and gameID=$gameID";
...
<?php		}
		...
		if((!$flag_team) && (!$flag_week))
		{
			...
			{ 
				...
				<script>
					...
					window.location="http://topherball.com/suicide_pick/pickteam.php";
				</script>
				<?php
			}
			else 
		...
		$userQuery="select gameID,team,`lock`,pickID from topherballpicks where userID = $userID order by team"; 
...
?>
	<script>
			alert("You can not pick any team as you are \"Deactivated\". Consult the Admin!");
			window.location = "http://topherball.com/suicide_pick/home.php";
	</script>
<?php	
		}
	}	
...
<body>
<a href="home.php">Back</a> 		 <a href="http://topherball.com/login.php">Logout</a><br>
</body>
</html>