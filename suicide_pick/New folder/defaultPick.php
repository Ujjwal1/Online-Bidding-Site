		...
		if($dbConnected)	
			{
				...
				{
					$sql_check_for_team = "select userID, team from topherballpicks where userID = $ID_user and team = \"$check_team\"";
...
<?php
				...
				if(isset($_GET['team']))
				{
					...
					$sql_user= "select distinct userID from topherballpicks where gameID = $gameID or pickID like \"$pick_like\"";
					...
					while($result_user = mysql_fetch_array($query_user))
						...
						$sql_remaining= "select userID from topherballusers where userID not in $userID";
										...
										if(!checkTeamForUser($remaining_userID,$team))
											{
												...
												$sql_user_insert ="insert into topherballpicks values ($remaining_userID , $gameID, \"$pick\",\"true\",\"$team\")";
...
<br><a href="http://topherball.com//index.php?login=success">Back to Home</a>