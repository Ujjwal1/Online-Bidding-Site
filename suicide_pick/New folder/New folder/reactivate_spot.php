	...
	if($isActivationWeek)
	{
		if($dbConnected)
		{
			if(isset($_GET['id']))
			{
				...
				$sql_verify = "update topherballspot set Activation = 3 where spotID = \"$id\" and Activation = 2";
				if(mysql_query($sql_verify))
				{
						...
						<script>
							...
							window.location="reactivate_spot.php";
						</script>
						<?php
							}
							else
			...
			$sql_spotted = "select * from topherballspot where Activation = 2 order by spotID";
			...
			{	echo "<table cellpadding=\"4\" cellspacing=\"0\" class=\"table1\"><tr><th>User Name</th><th colspan= '2'>Spot ID</th></tr>";
				...
				{	echo "<tr>";
					...
					echo "<td>$userName</td><td>$spotID</td>";?> <td> <button onClick = "spot('<?php echo $spotID; ?>')">Reactivate Spot</button></td><?php
			...
			{
				echo "<strong>No Spots to be reactivated</strong>";
			}
		}
		else
		{
...
<script>
	function spot(val)
	{
		window.location = "http://topherball.com/reactivate_spot.php?id="+val;
	}
</script>