...
if($dbConnected)	
{
	...
	if((isset($_GET['homeScore']))&&(isset($_GET['visitScore'])))
	{
			...
			if(!empty($homeScore) && !empty($visitScore))
			{
				...
				$lock = 1;
				$sql_result = "UPDATE topherball_schedule SET  homeScore=$homeScore , visitorScore=$visitScore , overtime=$overtime ,result=\"$result\",`lock` = $lock where gameID=$i_gameID";
				$query_result = mysql_query($sql_result) or die("Error while Updation.");
				if($query_result)
				{
					echo "Result Updated Successfully<br>";
					 $sql_active = "select userID, team from topherball_picks where gameID = $i_gameID";
					 ...
					 if(mysql_num_rows($query_active)>0)
					 {
						...
						$sql_team = "select teamID from topherball_teams where team =\"$team_active\"";
						...
						$sql_active_state = "select Activation,points from topherball_users where userID = $userID_active";
						...
						if(mysql_num_rows($query_active_state)>0)
						{
							...
							{		$points++;
									$sql_point ="update topherball_users set points=$points where userID = \"$userID_active\"";
									
									if(mysql_query($sql_point))
									{
										echo "<br>Point increased for userID ".$userID_active." and it turns to ".$points."<br>";
									}
									else
									{
							...
							{
									if($userID_active_state == 1)		//If deactivated for the first time
									{
										$userID_active_state = 2;
										mysql_query("update topherball_users set Activation=$userID_active_state where userID=$userID_active");
										echo $i_userID." <br>Some Ids have been Deactivated temporarily.<br>";
									}
									else
									if($userID_active_state == 3)		//Deactivated second time.
									{
										$userID_active_state = 4;
										mysql_query("update topherball_users set Activation=$userID_active_state where userID=$userID_active");
	...
	if((isset($_GET['home']))&&(isset($_GET['visit'])))
		{
			...
			$sql_home_team="select team from topherball_teams where teamID=\"$home_ID\"";
			...
			$sql_visit_team="select team from topherball_teams where teamID=\"$visit_ID\"";
...
if(isset($_GET['week']))
	...
	$week=$_GET['week'];
	//$weeklike=$week."_%";
	$sql_match ="select * from topherball_schedule where result is NULL and weekNum = \"$week\"";
	...
	while($result_match=mysql_fetch_array($query_match))
	{
		...
		$sql_home_team="select team from topherball_teams where teamID=\"$homeID\"";
			$query_home_team = mysql_query($sql_home_team);
			if(mysql_num_rows($query_home_team)>0)
			{
				$result_home_team = mysql_fetch_array($query_home_team);
			...
			$sql_visit_team="select team from topherball_teams where teamID=\"$visitorID\"";
			$query_visit_team = mysql_query($sql_visit_team);
			if(mysql_num_rows($query_visit_team)>0)
			{
				$result_visit_team = mysql_fetch_array($query_visit_team);
