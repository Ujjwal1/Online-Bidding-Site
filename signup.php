			...
			if ($password == $password2) {
				...
				if(mysql_numrows($result) > 0){
					...
					if(mysql_numrows($result) > 0){
						...
						//Creating the Spot
						$query = mysql_query("select userID from topherballusers where userName = \"".$username."\"") or die(mysql_error());
						if($query){
						$result = mysql_fetch_array($query);
						$userID = $result['userID'];
						$spotID = $username."-1";
						$sql_spot = "insert into ".$db_prefix."spot values(\"$spotID\",$userID,\"$username\",0)";
						...
						// html text block
						$mail->Body = '<p>Thank you for signing up for the NFL Pick \'Em Pool.  But Registration is yet to be verified. ' . "\n" . 
						...
						$sql_id = "select userID from topherballusers where userName = \"$username\"";
						$query_id = mysql_query($sql_id) or die("Can not get ID from the username");
						if(mysql_num_rows($query_id)>0)
						{ $result_id = mysql_fetch_array($query_id);
						$_SESSION['loginID'] = $result_id['userID'];}
